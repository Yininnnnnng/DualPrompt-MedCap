# -*- coding: utf-8 -*-
"""BLIP-2 image caption in json RAD ＋ SLAKE16/08/2024.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1BJ32Je_WG2DSScbzR1PZpotj5JMlnnM2
"""
...
def compute_loss(model, pixel_values, input_ids, attention_mask):
    outputs = model(
        ...
        labels=input_ids  # 关键修正：使用输入文本作为labels
    )
    return outputs.loss
...
def main():
    ...
    model.config.use_cache = False  # 减少内存使用

    # 数据集路径配置
    base_dir = "/content/drive/MyDrive/PhD/Research1/slakedataset/Slake1.0"
    ...
    # 训练循环
    best_loss = float('inf')
    model_save_path = '/content/drive/MyDrive/PhD/Research1/modelparasave/blip2_slake_best.pth'
    ...
    output_path = '/content/drive/MyDrive/PhD/Research1/output/slake_test_results.json'
...
def process_image_with_blip2(image_path, prompt=""):
    try:
        ...
        # 关键修复: 将问题作为提示而不是将其放入模型输入
        # BLIP-2不需要在输入中包含提示，会自动生成描述
        ...
        with torch.no_grad():
            generated_ids = model.generate(
                **inputs,
                max_new_tokens=100,    # 增加token数量以获得更详细的描述
                num_beams=5,           # 束搜索的数量
                min_length=20,         # 增加最小生成长度
                top_p=0.9,             # 核采样
